            ...
            if event.type == pygame.KEYDOWN:
                ...
                # Rift interaction: Charge/entry or toggle selection
                    ...
                    if self.locked_is_rift and self.locked_target is not None:
                        ...
                        avg_res = np.mean(self.resonance_levels)
                        if dist < RIFT_ALIGNMENT_TOLERANCE and avg_res > RIFT_ENTRY_RES_THRESHOLD / 2:
...
def audio_callback(outdata, frames, time, status):
    ...
    if power_condition and not any(np.array_equal(e.waveform, chord_waveform) for e in active_sound_effects):
